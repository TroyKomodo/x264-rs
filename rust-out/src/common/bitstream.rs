...
pub unsafe extern "C" fn x264_8_nal_encode(
    ...
    *fresh11 = ((0 as libc::c_int) << 7 as libc::c_int
    ...
    if (*h).param.i_avcintra_class != 0 {
        let mut padding: libc::c_int = (*nal).i_payload + (*nal).i_padding
            + 5 as libc::c_int - size;
        if padding > 0 as libc::c_int {
            memset(dst as *mut libc::c_void, 0 as libc::c_int, padding as libc::c_ulong);
            size += padding;
        }
        (*nal)
            .i_padding = if padding > 0 as libc::c_int {
            padding
        } else {
            0 as libc::c_int
        };
