...
pub unsafe extern "C" fn x264_8_threadpool_run(
    ...
    let mut job: *mut x264_threadpool_job_t = x264_8_sync_frame_list_pop(
        &mut (*pool).uninit,
    ) as *mut libc::c_void as *mut x264_threadpool_job_t;
    (*job).func = func;
    (*job).arg = arg;
    x264_8_sync_frame_list_push(
        &mut (*pool).run,
    ...
    loop {
        ...
        while i < (*pool).done.i_size {
            if (*(*((*pool).done.list).offset(i as isize) as *mut x264_threadpool_job_t))
                .arg == arg
            {
                let mut job: *mut x264_threadpool_job_t = x264_8_frame_shift(
