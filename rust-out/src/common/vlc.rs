...
extern "C" {
    ...
    static x264_run_before_init: [[vlc_t; 16]; 7];
}
#[derive(Copy, Clone)]
#[repr(C)]
...
#[no_mangle]
pub static mut x264_8_level_token: [[vlc_large_t; 128]; 7] = [[vlc_large_t {
    i_bits: 0,
    i_size: 0,
    i_next: 0,
}; 128]; 7];
#[no_mangle]
pub static mut x264_8_run_before: [uint32_t; 65536] = [0; 65536];
#[no_mangle]
pub unsafe extern "C" fn x264_8_cavlc_init(mut h: *mut x264_t) {
    ...
    while i_suffix < 7 as libc::c_int {
        ...
        while (level as libc::c_int) < 128 as libc::c_int / 2 as libc::c_int {
            ...
            let mut vlc: *mut vlc_large_t = &mut *(*x264_8_level_token
                .as_mut_ptr()
                .offset(i_suffix as isize))
                .as_mut_ptr()
                .offset(
                    (level as libc::c_int + 128 as libc::c_int / 2 as libc::c_int)
                        as isize,
                ) as *mut vlc_large_t;
            if i_level_code >> i_suffix < 14 as libc::c_int {
                (*vlc)
                    .i_size = ((i_level_code >> i_suffix) + 1 as libc::c_int + i_suffix)
                    as uint8_t;
                (*vlc)
                ...
                (*vlc)
                    .i_bits = (((1 as libc::c_int) << 4 as libc::c_int)
                    + (i_level_code - 14 as libc::c_int)) as uint16_t;
            } else if i_suffix > 0 as libc::c_int
                && i_level_code >> i_suffix == 14 as libc::c_int
            {
                ...
                (*vlc)
                    .i_bits = (((1 as libc::c_int) << 12 as libc::c_int) + i_level_code)
                    as uint16_t;
            }
    ...
    while i < ((1 as libc::c_int) << 16 as libc::c_int) as uint32_t {
        ...
        while j_0 < total - 1 as libc::c_int && zeros > 0 as libc::c_int {
            ...
            let mut len: libc::c_int = x264_run_before_init[idx as usize][run as usize]
                .i_size as libc::c_int;
            size += len;
            bits <<= len;
            bits
                |= x264_run_before_init[idx as usize][run as usize].i_bits
                    as libc::c_int;
            zeros -= run;
