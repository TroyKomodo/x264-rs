...
extern "C" {
    ...
    fn x264_8_sync_frame_list_push(
        slist: *mut x264_sync_frame_list_t,
...
pub unsafe extern "C" fn x264_8_lookahead_init(
    ...
    if !look.is_null() {
        ...
        (*look)
            .b_analyse_keyframe = (((*h).param.rc.b_mb_tree != 0
            || (*h).param.rc.i_vbv_buffer_size != 0 && (*h).param.rc.i_lookahead != 0)
            && (*h).param.rc.b_stat_read == 0) as libc::c_int as uint8_t;
            ...
            *look_h = *h;
            if x264_8_macroblock_cache_allocate(look_h) == 0 && x264_8_macroblock_thread_allocate(look_h, 1 as libc::c_int) >= 0 as libc::c_int && pthread_create(
                        ...
                        >(
                            ::core::mem::transmute::<
                                Option::<
                                    unsafe extern "C" fn(*mut x264_t) -> *mut libc::c_void,
                                >,
                                *mut libc::c_void,
                            >(
                                Some(
...
pub unsafe extern "C" fn x264_8_lookahead_is_empty(mut h: *mut x264_t) -> libc::c_int {
    pthread_mutex_lock(&mut (*(*h).lookahead).ofbuf.mutex);
    pthread_mutex_lock(&mut (*(*h).lookahead).next.mutex);
    let mut b_empty: libc::c_int = ((*(*h).lookahead).next.i_size == 0
