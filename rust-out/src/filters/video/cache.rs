...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct cache_hnd_t {
    pub prev_hnd: hnd_t,
    pub prev_filter: cli_vid_filter_t,
    pub max_size: libc::c_int,
    pub first_frame: libc::c_int,
    pub cache: *mut *mut cli_pic_t,
    pub cur_size: libc::c_int,
    pub eof: libc::c_int,
}
...
unsafe extern "C" fn fill_cache(mut h: *mut cache_hnd_t, mut frame: libc::c_int) {
    let mut shift: libc::c_int = frame
        - ((*h).first_frame + (*h).cur_size - 1 as libc::c_int);
    if shift <= 0 as libc::c_int || (*h).eof != 0 {
        return;
    }
    let mut cur_frame: libc::c_int = if (*h).first_frame + (*h).cur_size
    ...
    (*h)
        .first_frame = if (*h).first_frame + shift < cur_frame {
        (*h).first_frame + shift
    } else {
        cur_frame
    };
    (*h)
        .cur_size = if (*h).cur_size - shift > 0 as libc::c_int {
        (*h).cur_size - shift
    } else {
        0 as libc::c_int
    };
    while (*h).cur_size < (*h).max_size {
        ...
        if ((*h).prev_filter.get_frame)
            ...
            != 0 || x264_cli_pic_copy(cache, &mut temp) != 0
