...
unsafe extern "C" fn dither_plane_1(
    ...
    while y < height {
        ...
        while x < width {
            err = err * 2 as libc::c_int + *errors.offset(x as isize) as libc::c_int
            ...
            *dst
            ...
            err = *src.offset((x * 1 as libc::c_int) as isize) as libc::c_int
                - ((*dst.offset((x * 1 as libc::c_int) as isize) as libc::c_int)
                    << lshift);
        ...
        while x < width {
            ...
            *dst
            ...
            err = *src.offset((x * 2 as libc::c_int) as isize) as libc::c_int
                - ((*dst.offset((x * 2 as libc::c_int) as isize) as libc::c_int)
                    << lshift);
            *errors.offset(x as isize) = err as int16_t;
            x += 1;
            x;
        ...
        while x < width {
            ...
            *dst
            ...
            err = *src.offset((x * 3 as libc::c_int) as isize) as libc::c_int
                - ((*dst.offset((x * 3 as libc::c_int) as isize) as libc::c_int)
                    << lshift);
            *errors.offset(x as isize) = err as int16_t;
            x += 1;
            x;
        ...
        while x < width {
            ...
            *dst
            ...
            err = *src.offset((x * 4 as libc::c_int) as isize) as libc::c_int
                - ((*dst.offset((x * 4 as libc::c_int) as isize) as libc::c_int)
                    << lshift);
            *errors.offset(x as isize) = err as int16_t;
            x += 1;
            x;
...
unsafe extern "C" fn dither_image(
    ...
    while i < (*img).planes {
        ...
        let mut height: libc::c_int = ((*x264_cli_csps
            .as_ptr()
            .offset(csp_mask as isize))
            .height[i as usize] * (*img).height as libc::c_float) as libc::c_int;
        let mut width: libc::c_int = ((*x264_cli_csps.as_ptr().offset(csp_mask as isize))
            .width[i as usize] * (*img).width as libc::c_float
...
unsafe extern "C" fn scale_image(
    ...
    while i < (*img).planes {
        ...
        let mut height: libc::c_int = ((*x264_cli_csps
            ...
            .height[i as usize] * (*img).height as libc::c_float) as libc::c_int;
        let mut width: libc::c_int = ((*x264_cli_csps.as_ptr().offset(csp_mask as isize))
            .width[i as usize] * (*img).width as libc::c_float) as libc::c_int;
...
unsafe extern "C" fn get_frame(
    ...
    if (*h).bit_depth < 16 as libc::c_int
        && (*output).img.csp & 0x2000 as libc::c_int != 0
    {
        dither_image(&mut (*h).buffer.img, &mut (*output).img, (*h).error_buf);
        (*output).img = (*h).buffer.img;
    } else if (*h).bit_depth > 8 as libc::c_int
        && (*output).img.csp & 0x2000 as libc::c_int == 0
    {
...
unsafe extern "C" fn init(
    ...
    let mut change_fmt: libc::c_int = ((*info).csp ^ (*param).i_csp)
    ...
    if !opt_string.is_null() {
        ...
        if !opts.is_null() {
            let mut str_bit_depth: *mut libc::c_char = x264_get_option(
                b"bit_depth\0" as *const u8 as *const libc::c_char,
                opts,
            );
            bit_depth = x264_otoi(str_bit_depth, -(1 as libc::c_int));
            ret = (bit_depth < 8 as libc::c_int || bit_depth > 16 as libc::c_int)
                as libc::c_int;
    ...
    if change_fmt != 0 || bit_depth != 8 as libc::c_int * x264_cli_csp_depth_factor(csp)
    {
        ...
        let mut h: *mut depth_hnd_t = x264_malloc(
            (::core::mem::size_of::<depth_hnd_t>() as libc::c_ulong)
                .wrapping_add(
