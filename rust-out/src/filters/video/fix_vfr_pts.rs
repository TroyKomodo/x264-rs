...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct fix_vfr_pts_hnd_t {
    pub prev_hnd: hnd_t,
    pub prev_filter: cli_vid_filter_t,
    pub buffer: cli_pic_t,
    pub holder: cli_pic_t,
    pub buffer_allocated: libc::c_int,
    pub holder_frame: libc::c_int,
    pub holder_ret: libc::c_int,
    pub pts: int64_t,
...
unsafe extern "C" fn get_frame(
    ...
    if frame == (*h).holder_frame {
        ...
        if (*h).holder_frame > 0 as libc::c_int && (*h).holder_frame < frame
            && ((*h).prev_filter.release_frame)
                .expect(
                    "non-null function pointer",
                )((*h).prev_hnd, &mut (*h).holder, (*h).holder_frame) != 0
        {
            return -(1 as libc::c_int);
        }
        (*h).holder_frame = -(1 as libc::c_int);
    ...
    if (*h).holder.duration == 0 {
        ...
        if x264_cli_pic_copy(&mut (*h).buffer, &mut (*h).holder) != 0
            || ((*h).prev_filter.release_frame)
                .expect(
                    "non-null function pointer",
                )((*h).prev_hnd, &mut (*h).holder, frame) != 0
        {
            return -(1 as libc::c_int);
        }
        (*h)
            .holder_ret = ((*h).prev_filter.get_frame)
            .expect(
                "non-null function pointer",
            )((*h).prev_hnd, &mut (*h).holder, (*h).holder_frame);
        if (*h).holder_ret == 0 {
            (*h)
                .last_duration = if (*h).holder.pts - (*h).buffer.pts
